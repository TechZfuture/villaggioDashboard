...
async function excluirRegistrosAusentesNoBancoDeDados(dataFromAPI) {
    ...
    try {
        ...
        const [existingRecords] = await connection.execute("SELECT schedule_id FROM past_due_payments");
        ...
        for (const id of existingRecordIds) {
            if (!dataFromAPI.some(item => item.scheduleId === id)) {
                ...
                await connection.execute("DELETE FROM past_due_payments WHERE schedule_id = ?", [id]);
