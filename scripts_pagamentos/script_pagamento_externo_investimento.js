    ...
    for (const item of data) {
      if (item.isTransfer === false) {
        const [existe] = await connection.execute(
          `SELECT value FROM external_payments WHERE category_parent_id = "ddfca14f-e90f-48da-91bb-f9f8ccceb90f"`
        );
        if (existe.length > 0) {
          await connection.execute(
            `UPDATE external_payments SET value_investiment = ? where entry_id = ? AND category_parent_id = "ddfca14f-e90f-48da-91bb-f9f8ccceb90f"`,
            [existe[0].value, item.entryId]
          );
        }
      }
    }
